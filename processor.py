...
strclient = storage.Client(project='project-id')
bucket = strclient.get_bucket('outputbucket')

# Test processors
document_ocr_display_name = "document-ocr1"
form_parser_display_name = "document-ocr"
...
try:
    ...
    text = "\n".join(lines)
    blob = bucket.blob(f'{file_env}processedfile.txt')
    blob.upload_from_string(text)
    
except:
    error = "This file is not suitable for the covertion through DOC AI, please try to open this on your system and check out whether the file is corrupted"
    print(error)
    error_blob_name = f'{file_env}_error'
    error_blob = bucket.blob(error_blob_name)
    error_blob.upload_from_string(error)
